...
int graphHeight;
int xBars, xSpacer;
float maxBar, maxLine, minLine, minBar;
...
void getHeader() {
  ...
  graphHeader = "Bars =" + header[2] + " and Line = " + header[1];
  graphX = header[0];
}

void processTwoVisOneChart() {
  if (maxBar > maxLine) {
    maxOverall = floor(maxBar); //Using floor to get the lower val
  } else {
    maxOverall = floor(maxLine); //Using floor to get the lower val
  }
  ...
  //  minOverall = minLine;
  //}
  minOverall = 0; //Because barChart should always start with zero :)
}
...
void processLineChart() {
  ...
  for (int i=0; i<column1.length; i++) {
    ...
    float adjustedY = map(column1[i], minOverall, maxOverall, 0, graphHeight); //from 0 to 10...
    float yPosition = height - buffer - adjustedY;
    //float xPosition = buffer + (xBars *i);
    rectWidth = (width-buffer-buffer)/column2.length;

    float xPosition = buffer + (rectWidth*i) + rectWidth/2 ;
