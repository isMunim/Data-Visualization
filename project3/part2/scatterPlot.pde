...
class scatterPlot extends frame {
  ...
  void drawGrid(){ 
    ...
    for (int i=0; i <= 10; i++){
      //Ten text labels  
      textAlign(LEFT);
      text((nf(((minY)+(i*incrementY)),0,2)),5,(canvasHeight-(i*diffY)));
      //text label lines
      stroke(200,100);
    ...
    for (int i=0; i<=10; i++){
      ...
      textAlign(LEFT);
      text((nf(((minX)+(i*incrementX)),0,2)),(buffer+(i*diffX)),height-15);
    ...
    //Draws a 10x10 grid in our graph area.
    int diffY = (canvasHeight-buffer)/10;
    float incrementX = ((maxX-0)/10);
    float incrementY = ((maxY-0)/10);
    for (int i=0; i <= 10; i++){
      //Ten text labels  
      textAlign(LEFT);
      text(((nf((i*incrementY),0,2))),5,(canvasHeight-(i*diffY)));
      //text label lines
      stroke(200,100);
      line(buffer,(canvasHeight-(i*diffY)),canvasWidth, (canvasHeight-(i*diffY)) );
    }
    ...
    for (int i=0; i<=10; i++){
      ...
      //labels for ten lines
      textAlign(LEFT);
      text((nf((i*incrementX),0,2)),(buffer+(i*diffX)),height-15);
    }
  }
